...
There is no bootc host available to boot in a normal dev/CI environment, so none of the above proves the login flow actually works end-to-end. A real smoke test needs a VM: build a bootable disk image with `bootc-image-builder`, boot it under libvirt/qemu with a console, create a test user, and confirm at the `greetd`/`agreety` prompt that logging in creates and enters a `desktop-env-<user>` container running `startup.sh` (which will correctly report "No Window Manager found" until a WM is `dnf install`ed inside that container).
